...
void MRG::run(MRG_REAL dur, const char * in_file, MRG_REAL V_fe, MRG_REAL V_applied,
              MRG_REAL stim_start, MRG_REAL stim_end)
{
  ...
  t.reserve(int(dur / dtout + 0.5) + 1);
  Y = MRG_MATRIX_REAL(IC.n_rows, int(dur / dtout + 0.5) + 1);
  Y.col(0) = IC;

  unsigned int i = 1;
  for (MRG_REAL tout = dtout; tout < dur; tout += dtout, ++i) {
    realtype t1;
    N_Vector Y1 = y0;
...
int MRG::odeMcIntyr(realtype t, N_Vector y, N_Vector ydot) const
{
  ...
  const MRG_MATRIX_REAL & V_e = (t >= m_stim_start && t <= m_stim_end) ? V_stim
                                : arma::zeros<MRG_MATRIX_REAL>(i_inter[5][1] + 1, 1);
  Xlr = (t >= m_stim_start && t <= m_stim_end) ? Xlr_stim
