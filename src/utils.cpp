...
bool is_monotonically_increasing(const MRG_MATRIX_REAL & x)
{
  auto prev_iter = x.begin();

  for (auto iter = prev_iter + 1; iter != x.end(); ++iter) {
    if (*prev_iter > *iter) {
      return false;
    }
  }
...
/** To inter-pole data */
// actually extrapolates as well, assume x and xi are monotonically increasing
void MRG_interp1(const MRG_MATRIX_REAL & x, const MRG_MATRIX_REAL & y, const MRG_MATRIX_REAL & xi, MRG_MATRIX_REAL & yi)
{
  ...
  assert(is_monotonically_increasing(x) && is_monotonically_increasing(xi));
